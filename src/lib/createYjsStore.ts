...
export default function <T extends { [key: string]: any }>(
  ...
  const optimizeMutationOperations = (
    ...
    LOG(['optimizeMutationOperations', 'deleted is'], deleted)
    ...
    LOG(['optimizeMutationOperations', 'added is'], added, ops.added)
  ...
  const solidToYjs = (
    ...
    LOG(['solidToYjs'], yparent, sparent, key, newValue)
    ...
    tree[tree.length - 1].forEach(({ sparent, yparent }) => {
      ...
      if (typeof nextValue === 'function') {
        ...
        if (nextValueToString === produceToString) {
          ...
          nextValue(proxy)
          console.log('PROXY is ', proxy, proxy[$PROXY])
          ...
          Array.from(dirty.entries()).forEach(([svalue, properties]) => {
            ...
            properties.forEach((property) => {
              console.log(yparent, svalue, property, svalue[property])
