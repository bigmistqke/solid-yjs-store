...
export default function reconcile(
  ...
  const adjust = (target, parent, property) => {
    ...
    } else if (parent instanceof Y.Map && typeof property === 'string') {
  ...
  const compare = (
    ...
    if (
      (parent instanceof Y.Map || parent instanceof Y.Doc) &&
    ...
    const previous =
      yPrevious instanceof Y.AbstractType ? yPrevious.toJSON() : yPrevious

    console.log('target previous', target, previous)

    if (Array.isArray(target)) {
      // remove excess elements
      if (previous.length > target.length) {
        console.log('this happens')
        yPrevious.delete(target.length, previous.length - target.length)
      }
